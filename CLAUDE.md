...
| 🐦 X推文档案库 | `4b321d0adf604c8bbb443e04c65d9bdb` |

### 小咪 scheme

| 库 | database_id |
|----|-------------|
| 源库 | `ec28335c510183d2a1ba01d62798874b` |
| 维基库 | `e1b8335c5101836bb60f81286f082229` |
| 日志库 | `da58335c510183eeaaa281337dca11d4` |
...
**源库层（wiki_lint/lint.py）：**（含 X推文档案库）
9. 长期待摄入 — 状态=待摄入 且 加入超过7天
10. 孤儿源 — 状态=已摄入 但无「相关维基页」关联

**质量层（wiki_lint/lint.py）：**
11. 字段完整性 — 信号分/目标对齐/依据源 为空，或有标记/相关页时关系说明为空
12. 错字检查 — Gemini 扫描标题和一句话摘要，找明显汉字错别字

---

## X推文两阶段摄入 SOP（x scheme）

摄入 X（Twitter）内容时，**强制两步走**：

### 第一步：原文存档到 X推文档案库

在 Notion X推文档案库（`4b321d0adf604c8bbb443e04c65d9bdb`）创建新页面，字段全部填写：

| 字段 | 内容 |
|------|------|
| 条目（title） | 推文标题或首句（≤30字） |
| 状态（select） | `待摄入` |
| 平台（select） | `X/Twitter` |
| 原文链接（url） | 推文 URL |
| 发布日期（date） | 推文发布日期 |
| 作者（text） | @用户名 |
| 标签（multi_select） | 根据主题打标（如 LLM、创作、工具等） |
| 原始内容（text） | 完整推文正文，不裁剪 |

> 目的：保存完整原始内容，不做分类和摘要，作为摄入前缓冲池。

### 第二步：按 scheme 约定摄入到维基库

完成存档后，立即执行标准摄入流程（见 x scheme 约定页）：
1. 阅读原文，提取核心洞见
2. 在维基库创建或更新对应主题页
3. 将 X推文档案库页面的**状态改为 `已摄入`**，并填写「相关维基页」关联
4. 写日志（`--op 摄入`）
