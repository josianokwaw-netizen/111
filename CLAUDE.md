...
| 总目录页 | `3888335c510181b5bb8ce93030061a22` |

### 小咪

| 库 | database_id |
|----|-------------|
| 维基库 | `e1b8335c5101836bb60f81286f082229` |
| 总目录页 | `3888335c510181c9a83edd9d108c2d2d` |

### 投资

| 库 | database_id |
|----|-------------|
| 维基库 | `1aaf457a389f439880a18255de0089d9` |
| 总目录页 | `3888335c510181c78aa3c7d6c9f3c07d` |
...
### Ingest 完整流程（每次摄入必须跑完以下步骤）

一次 Ingest 应触及 **5-10 个维基页**，不只是新建摘要页：

1. **读源 + 过要点**：与用户确认核心观点和分数（四轴打分）
2. **新建/更新摘要页**：填「页面类型=摘要」「一句话摘要」「依据源」（必填，不得为空）
3. **回填源页**：在源库该条目的「相关维基页」字段关联新摘要页
4. **更新相关实体/概念页**：新源提到的实体（人物/工具/策略）或概念 → 找现有页并更新内容/添加相关页链接
5. **检查矛盾**：新源论点与现有页面是否有冲突 → 有则在相关页标 `⚠️有矛盾`，关系说明写明分歧点
6. **更新总目录页**：在各库「🗂️ 总目录」页面的对应章节追加新页条目 + 一句话摘要
7. **写操作日志**：操作=摄入，详情记录新建页 URL

### Query 好答案回填规则

每次 Query 结束后判断是否回填（**满足其一即必须回填**）：
- 综合答案 > 200 字
- 发现了现有页面之间的新关联
- 识别出新实体/概念值得独立成页

**回填步骤：**
1. 新建「问答」类型页（标题格式：`Q&A：[问题关键词]`）
2. 填「一句话摘要」（核心结论）、「依据源」（参考了哪些源）、「相关页」（引用了哪些维基页）
3. 写操作日志（操作=查询，详情记录回填页 URL）

### 矛盾追踪协议

**Ingest 时（实时检测）：**
- 摄入新源时必须与现有相关页对比，发现冲突即在维基页标 `⚠️有矛盾`
- 关系说明格式：`⚠️ 分歧：《来源A》vs《来源B》—— A认为[论点]；B认为[论点]。分歧根源：[前提差异]。我的判断：[倾向]`

**Lint 时（定期汇总）：**
- 扫描所有 `⚠️有矛盾` 页 → 更新「🗂️ 总目录」中的「活矛盾表」章节

### 总目录页约定（index.md 等价物）

每个维基库维护一个固定页「**🗂️ 总目录**」，是 Query 时 LLM 最先读取的导航页：
- 按「页面类型」分章节列出所有页面
- 每条格式：`- [页面标题](URL) — 一句话摘要`
- Ingest 后必须更新
- 活矛盾表作为最后一个章节列在页末
...
- `摄入这个` → Ingest 流程（完整7步）→ 写日志
- `关于 X，Wiki 里怎么说？` → Query 流程 → 好答案必须回填 → 写日志
